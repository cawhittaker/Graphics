...
void Renderer::RenderScene() {
	...
	for (unsigned int i = 0; i < 2; ++i) {
		//Vector3 test(0, 0, -10);
		//Matrix4 mTest = Matrix4::Translation(test);

		//int uTest = glGetUniformLocation(currentShader->GetProgram(), "modelMatrix");

		//glUniformMatrix4fv(glGetUniformLocation(currentShader->GetProgram(), "modelMatrix"), 1, false, (float*)&test);
